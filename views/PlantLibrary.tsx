...
import { Species, Plant } from '../types';
import { storageService } from '../services/storageService';
import { ChevronLeft, Search, Plus, Calendar, X, Droplets, Info, Sparkles, Sprout } from 'lucide-react';
...
const PlantLibraryView: React.FC<PlantLibraryViewProps> = ({ onBack, onAdd }) => {
  ...
  if (selectedSpecies && !showAddModal) {
    ...
    return (
      <div className="flex flex-col min-h-screen bg-[#fdfdfb]">
        <header className="p-5 flex items-center gap-3 border-b border-stone-50 bg-white sticky top-0 z-30">
          <button onClick={() => setSelectedSpecies(null)} className="p-1 -ml-1 text-stone-400"><ChevronLeft size={28} /></button>
          <div>
            <h1 className="text-xl font-bold text-stone-800">{selectedSpecies.name}</h1>
        ...
        <div className="p-6 space-y-6">
          {/* Main Info Card */}
          <div className="bg-white border border-stone-100 p-8 rounded-[40px] shadow-sm flex flex-col items-center gap-6 relative overflow-hidden">
             <div className="absolute top-0 right-0 w-32 h-32 bg-stone-50 rounded-full -mr-16 -mt-16 opacity-30"></div>
             <span className="text-7xl mb-2">{selectedSpecies.emoji}</span>
             <div className="text-center">
                <h2 className="text-2xl font-black text-stone-800 mb-1">{selectedSpecies.name}</h2>
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  ...
                  <span className={`px-4 py-1.5 text-white text-[11px] font-bold rounded-full ${selectedSpecies.difficulty === 'easy' ? 'bg-[#559a73]' : 'bg-red-400'}`}>{selectedSpecies.difficulty}</span>
                  <span className="px-4 py-1.5 bg-white border border-stone-100 text-stone-400 text-[11px] font-bold rounded-full flex items-center gap-1.5">
                    <Calendar size={12} /> {selectedSpecies.durationDays} days
                  </span>
                </div>
             </div>
             <p className="text-stone-500 text-sm leading-relaxed text-center px-4">
          ...
          <div className="grid grid-cols-1 gap-6">
            {/* Water Calculator */}
            <div className="bg-white border border-stone-100 p-6 rounded-[32px] shadow-sm space-y-6">
              <div className="flex items-center gap-2">
                <div className="bg-blue-50 p-2 rounded-xl text-blue-500">
                  <Droplets size={20} />
                </div>
                <h3 className="text-lg font-black text-stone-800">Water Calculator</h3>
              </div>
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex gap-2 bg-stone-50 p-1 rounded-2xl">
                    {['Small', 'Medium', 'Large'].map(size => (
                      <button 
                        ...
                        className={`flex-1 py-2 text-xs font-bold rounded-xl transition-all ${potSize === size ? 'bg-white text-stone-800 shadow-sm' : 'text-stone-400'}`}
                      >
                        {size}
                      </button>
                    ))}
                  ...
                  <div className="flex gap-2 bg-stone-50 p-1 rounded-2xl">
                    {['Low', 'Moderate', 'Full'].map(sun => (
                      <button 
                        ...
                        className={`flex-1 py-2 text-xs font-bold rounded-xl transition-all ${sunExposure === sun ? 'bg-white text-stone-800 shadow-sm' : 'text-stone-400'}`}
                      >
                        {sun}
                      </button>
                    ))}
                ...
                <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100 flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-sm font-black text-blue-800">{waterInfo.frequency}</p>
                  </div>
                  <div className="text-right space-y-0.5">
                    <p className="text-[10px] font-bold text-blue-400 uppercase tracking-tight">Amount</p>
                    <p className="text-sm font-black text-blue-800">{waterInfo.amount}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-stone-100 p-6 rounded-[32px] shadow-sm space-y-6">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-black text-stone-800">Harvest Calculator</h3>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-2">
                  ...
                  <input 
                    type="date" 
                    className="w-full bg-stone-50 p-4 rounded-2xl border-none focus:ring-0 text-sm font-bold text-stone-700"
                ...
                <div className="bg-amber-50/50 p-5 rounded-2xl border border-amber-100 flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-amber-600 uppercase tracking-widest">Ready to harvest around</p>
                    <p className="text-base font-black text-amber-900">{getEstimatedHarvest()}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white border border-stone-100 p-6 rounded-[32px] shadow-sm">
            <h3 className="text-lg font-black text-stone-800 mb-8">Growth Stages</h3>
            <div className="space-y-10 relative pl-8">
              <div className="absolute left-[15px] top-2 bottom-2 w-0.5 bg-stone-100"></div>
              {selectedSpecies.stages.map((st, i) => (
                <div key={i} className="relative">
                  <div className={`absolute -left-[32px] w-8 h-8 rounded-full border-4 border-white shadow-md flex items-center justify-center z-10 ${i === 0 ? 'bg-emerald-500' : 'bg-stone-200 opacity-60'}`}>
                    <span className="text-sm">{st.emoji}</span>
                  </div>
                  <div className="space-y-1">
                    <h4 className={`text-sm font-bold ${i === 0 ? 'text-[#559a73]' : 'text-stone-800 opacity-50'}`}>{st.stage} <span className="text-[10px] text-stone-400 font-medium ml-2">Days {st.days}</span></h4>
                    <p className={`text-xs leading-relaxed ${i === 0 ? 'text-stone-600' : 'text-stone-400 opacity-50'}`}>{st.instruction}</p>
                    {i === 0 && <span className="inline-block mt-2 px-3 py-1 bg-red-50 text-red-400 text-[10px] font-bold rounded-full">📍 Currently at Day 0</span>}
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#fdfdfb] pb-10">
      <header className="p-5 flex items-center gap-3 border-b border-stone-50 bg-white">
        <button onClick={onBack} className="p-1 -ml-1 text-stone-400"><ChevronLeft size={28} /></button>
        <div>
          <h1 className="text-xl font-black text-stone-800">Plant Library</h1>
      ...
      <div className="p-5 space-y-5">
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-[#f4f4f4] py-4 pl-12 pr-4 rounded-2xl focus:outline-none text-sm placeholder:text-stone-400"
            value={search}
            ...
            return (
              <button
                ...
                className={`flex-1 py-3 text-xs font-bold rounded-xl transition-all ${
                  activeFilter === f ? 'bg-white text-stone-800 shadow-sm' : 'text-stone-400'
                }`}
              >
                {f} ({count})
              </button>
        ...
        <div className="space-y-4">
          {filtered.map(species => (
            <div 
              ...
              className="bg-white border border-stone-100 p-5 rounded-[32px] shadow-sm flex flex-col gap-4 active:scale-[0.98] transition-all relative overflow-hidden"
            >
              <div className="flex gap-5">
                <span className="text-5xl">{species.emoji}</span>
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <h3 className="font-black text-stone-800 tracking-tight">{species.name} <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest ml-2">{species.difficulty}</span></h3>
                  </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[40px] p-8 relative animate-in zoom-in duration-200 shadow-2xl">
            <button onClick={() => setShowAddModal(false)} className="absolute top-6 right-6 text-stone-400"><X size={20} /></button>
            <h2 className="text-xl font-black text-center mb-8 text-stone-800">Add {selectedSpecies?.name} to Your Garden</h2>
            <div className="space-y-6">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-[#f4f4f4] p-4 rounded-2xl border border-transparent focus:border-emerald-500/20 focus:outline-none text-sm"
                  value={nickname}
              ...
              <div className="p-3 bg-stone-50 rounded-2xl border border-stone-100 flex gap-3">
